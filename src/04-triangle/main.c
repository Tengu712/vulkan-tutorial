...
int main() {
    ...
    } model;
    {
        ...
        const size_t vtxs_size = sizeof(float) * 3 * 3;
        const size_t idxs_size = sizeof(uint32_t) * 3;
        model.index_cnt = 3;
        // NOTE: 各バッファを作る。
        CHECK_VK(
            create_buffer(
            ...
            "failed to create a vertex buffer for a model."
        );
        CHECK_VK(
            create_buffer(
                device,
            ...
            "failed to create a index buffer for a model."
        );
        // NOTE: 各バッファにマップする。
        CHECK_VK(map_memory(device, model.vtx_memory, (void *)vtxs, vtxs_size), "failed to map vertex buffer data for a model.");
        CHECK_VK(map_memory(device, model.idx_memory, (void *)idxs, idxs_size), "failed to map index buffer data for a model.");
    }
